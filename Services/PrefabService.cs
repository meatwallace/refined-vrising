...
internal class PrefabService
{
	...
	internal PrefabService()
	{
		...
		Core.Logger.LogDebug($"All prefabs: {allPrefabs.Count}");
		...
		foreach (var kvp in allPrefabs)
		{
			...
			if (!success)
			{
				Core.Logger.LogDebug($"{kvp.Key} exist already, skipping.");
			}
		}

		var spawnable = collectionSystem.SpawnableNameToPrefabGuidDictionary;

		Core.Logger.LogDebug($"Spawnable prefabs: {spawnable.Count}");
